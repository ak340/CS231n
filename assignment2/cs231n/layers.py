...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dx = dout.dot(w.T).reshape(np.shape(x))
...
def relu_forward(x):
    ...
    ###########################################################################
    out = np.maximum(np.zeros(np.shape(x)),x)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = x
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xmu = x - sample_mean
        sqrd = xmu**2
        sample_var = 1 / float(N) * np.sum(sqrd, axis=0)
        ...
        cache = (sample_mean, xmu, sqrd, sample_var, eps, std, invstd, x_norm, gamma, beta, x)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    sample_mean, xmu, sqrd, sample_var, eps, std, invstd, x_norm, gamma, beta, x= cache
    N, D = dout.shape
    ...
    dnorm = dout*gamma
    dxmu = invstd * dnorm
    dinvstd = np.sum(xmu * dnorm, axis=0)
    dstd = -1.0 / (std**2) * dinvstd
    dvar = 0.5 * (sample_var + eps)**(-0.5) * dstd
    dsqrd = 1.0 / N * np.ones((sqrd.shape)) * dvar
    dxmu += 2 * xmu * dsqrd
    dx = dxmu
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    mean, xmu, sqrd, var, eps, std, invstd, x_norm, gamma, beta, x= cache
    N, D = dout.shape
...
def conv_backward_naive(dout, cache):
    ...
    stride = conv_param['stride']
    pad = conv_param['pad']
    h_prime = dout.shape[2]#int(1+(H+2*pad-HH)/stride)
    w_prime = dout.shape[3]#int(1+(W+2*pad-WW)/stride)
    ###########################################################################
        ...
        for c in range(C):
            for h in range(height):
                for w in range(width):
                    out[n,c,h,w] = np.max(x[n,c,h*stride:(h*stride+HH),w*stride:(w*stride+WW)])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(height):
                for w in range(width):
                    mx = x[n,c,h*stride:(h*stride+HH),w*stride:(w*stride+WW)] 
                    (i,j) = np.unravel_index(np.argmax(mx, axis=None), mx.shape)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta = batchnorm_backward_alt(dout.reshape(C,N*H*W).T, cache)
